...
import { customerApiBase, customerApi } from "@/api/customerApi";
...
const staticCatalogProducts: Product[] = catalogProducts.map((product, index) => ({
  id: product.id,
  name: product.name,
  subtitle: product.subtitle || product.description,
  price: product.price,
  rating: [4.6, 4.9, 4.7, 4.8, 4.5, 4.8][index % 6],
  badge: index === 0 ? "الأكثر طلباً" : index === 2 ? "حصري" : index === 4 ? "جديد" : "مميز",
  image: product.image,
...
export function ProductsPage() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        // Prefer using customerApi which handles auth header injection.
        const resp = await customerApi.get('/products');
        const data = resp.data;
n        const items = unwrapProductsPayload(data);
n        if (items.length > 0 || (data && typeof data === 'object' && Object.keys(data as Record<string, unknown>).length > 0)) {
          const normalized = items.map((item) => normalizeProductRecord(item));
          if (isMounted) {
            setProducts(normalized.length > 0 ? normalized : staticCatalogProducts);
          }
        } else {
          if (isMounted) setProducts(staticCatalogProducts);
        }
      } catch (fetchError) {
        console.warn('Using local catalog data because the product API is unavailable.', fetchError);
        if (isMounted) {
          setProducts(staticCatalogProducts);
          setError(null);
        }
      } finally {
