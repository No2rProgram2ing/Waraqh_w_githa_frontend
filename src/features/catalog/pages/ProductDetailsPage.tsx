        ...
        <div className="mt-6 border-t border-[var(--color-border)] pt-6">
          <div className="mb-4">
            ...
            <p className="mt-1 text-xs text-[var(--color-text-muted)]">
              الخصائص والقيم المرتبطة بهذا المنتج.
            </p>
          </div>
          ...
          {!product.attributes?.length ? (
            <div className="rounded-xl border border-dashed border-[var(--color-border)] bg-[var(--color-surface)] p-5 text-center">
              <p className="text-sm text-[var(--color-text-muted)]">
                لا توجد خصائص مرتبطة بهذا المنتج حاليًا.
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-[var(--color-text-muted)]">
                        الاسم التقني: {attribute.name}
                      </p>
                    </div>

                    <span className="shrink-0 rounded-full bg-[var(--color-surface-subtle)] px-2.5 py-1 text-xs font-medium text-[var(--color-text-muted)]">
                      {getInputTypeLabel(attribute.input_type)}
                    </span>
                  </div>

                  <div className="mt-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-card)] px-4 py-3">
                    <p className="text-xs text-[var(--color-text-muted)]">
                      القيمة
                    </p>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    {attribute.is_required ? (
                      <span className="rounded-full bg-[var(--color-accent-subtle)] px-2.5 py-1 text-xs font-semibold text-[#45592D]">
                        خاصية مطلوبة
                      </span>
                    ) : (
                      <span className="rounded-full bg-[var(--color-surface-subtle)] px-2.5 py-1 text-xs font-medium text-[var(--color-text-muted)]">
                        اختيارية
                      </span>
                    )}
                    ...
                    {attribute.input_type === 'select' &&
                      attribute.options?.length ? (
                        <span className="rounded-full bg-[var(--color-surface-subtle)] px-2.5 py-1 text-xs text-[var(--color-text-muted)]">
                          {attribute.options.length} خيارات
                        </span>
                      ) : null}
                  </div>
                </div>
