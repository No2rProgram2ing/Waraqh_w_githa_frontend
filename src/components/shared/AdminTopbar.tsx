...
function AdminTopbar() {
  ...
  const { data: globalResults, isLoading } = useGlobalSearch(debouncedQuery)
        ...
        >
          ...
          {isDropdownOpen && (
            <div className="absolute inset-x-0 top-full z-40 mt-2 overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-card)] shadow-lg">
              {isLoading ? (
                <div className="px-4 py-3 text-sm text-[var(--color-text-muted)]">جارٍ البحث...</div>
              ) : hasResults ? (
                <div className="max-h-80 overflow-y-auto">
                  {searchGroups.map((group) => {
                    ...
                    return (
                      <div key={group.title} className="border-b border-[var(--color-border)] last:border-b-0">
                        ...
                        <ul>
                          {group.items.map((item) => {
                            ...
                            const secondaryText = group.getSecondary(item as never)

                            return (
                              <li key={`${group.title}-${id}`}>
                                <button
                                  type="button"
                                  onClick={() => handleSelectResult(group.path(id))}
                                  className="flex w-full items-center justify-between gap-2 px-4 py-2 text-right transition-colors hover:bg-[var(--color-surface-subtle)]"
                                >
                                  <div className="min-w-0">
                                    <div className="truncate text-sm font-medium text-[var(--color-text-primary)]">{label}</div>
                                    {secondaryText ? (
                                      <div className="truncate text-xs text-[var(--color-text-faint)]">{secondaryText}</div>
                                    ) : null}
                                  </div>
                                </button>
                              </li>
                ...
                <div className="px-4 py-3 text-sm text-[var(--color-text-muted)]">لا توجد نتائج مطابقة</div>
              )}
            </div>
          )}
        </div>
